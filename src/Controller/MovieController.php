...
public function AncientgetMovie( int $idMovie) : Response
{
    ...
    {
      $output = new Response(
        "Vous avez demandé un film inexistant :". $idMovie,
        404
      );
    }
    return $output;
...
/**
 * @Route("/movies/{slug}", name="movie_name" ,
 *  methods={"GET"}), requirements={"slug"="\W+"})
 */
public function getMoviebySlug(string $slug): Response
{
  // resultat -> récupère la liste des movies slug ou un tableau vide 
  $resultat = $this->getMovieListBySlug($slug);
  if (count ($resultat) == 0)
  {
    $output = new Response ("Aucun film trouvé avec le slug: " . $slug,404);

  }
  else
  {
    $output = $this ->render('movie/getMovieSlug.html.twig',
    [
      'movies' => $resultat
    ],);
  }
...
/**
 * @Route("/displayJSON", name="display_JSON" ,
 *  methods={"GET"})
 */
public function displayJSON() : Response
{
  return new JsonResponse([$this->_movies]);
}
}